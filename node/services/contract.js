...
const totalSupplyABI = Solidity.arc20ABIs.find(abi => abi.name === 'totalSupply')
const balanceOfABI = Solidity.arc20ABIs.find(abi => abi.name === 'balanceOf')
const ownerOfABI = Solidity.arc721ABIs.find(abi => abi.name === 'ownerOf')
...
class ContractService extends Service {
  ...
  async start() {
    ...
    this.#ARC20 = this.node.getModel('arc20')
    this.#ARC20Balance = this.node.getModel('arc20_balance')
    this.#ARC721 = this.node.getModel('arc721')
    this.#ARC721Token = this.node.getModel('arc721_token')
    ...
    for (let transaction of block.transactions) {
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          if (contract && contract.type === 'arc20') {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      await this.#db.query(sql`
        DELETE contract, tag, arc20, arc20_balance, arc721, arc721_token
        FROM contract
        LEFT JOIN contract_tag tag ON tag.contract_address = contract.address
        LEFT JOIN arc20 ON arc20.contract_address = contract.address
        LEFT JOIN arc20_balance ON arc20_balance.contract_address = contract.address
        LEFT JOIN arc721 ON arc721.contract_address = contract.address
        LEFT JOIN arc721_token ON arc721_token.contract_address = contract.address
  ...
  async _createContract(address, vm) {
    ...
    if (isARC721(code)) {
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([
        {address, abi: Solidity.arc721ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.arc721ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.arc721ABIs.find(abi => abi.name === 'totalSupply')}
      ])
      try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'arc721'})
        await this.#ARC721.create({
    ...
    } else if (isARC20(code)) {
      let [
        nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult
      ] = await this._batchCallMethods([
        {address, abi: Solidity.arc20ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.arc20ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.arc20ABIs.find(abi => abi.name === 'decimals')},
        {address, abi: Solidity.arc20ABIs.find(abi => abi.name === 'totalSupply')},
        {address, abi: Solidity.arc20ABIs.find(abi => abi.name === 'version')}
      ])
      try {
        let version
        try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'arc20'})
        await this.#ARC20.create({
          contractAddress: address,
          name,
          symbol,
          decimals,
    ...
    for (let addressString of totalSupplyChanges) {
      ...
      if (contract) {
        ...
        if (contract.type === 'arc20') {
          await this.#ARC20.update({totalSupply}, {where: {contractAddress: address}})
        } else {
          await this.#ARC721.update({totalSupply}, {where: {contractAddress: address}})
        }
      }
    }
  }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await this.#ARC20Balance.bulkCreate(operations, {updateOnDuplicate: ['balance'], validate: false})
    }
  }
  ...
  async _updateTokenHolders(transfers) {
    ...
    await this.#ARC721Token.bulkCreate(operations, {updateOnDuplicate: ['holder'], validate: false})
  }
}

function isARC20(code) {
  return code.includes(balanceOfABI.id)
    && code.includes(transferABI.id)
    && code.includes(TransferABI.id)
}

function isARC721(code) {
  return code.includes(balanceOfABI.id)
